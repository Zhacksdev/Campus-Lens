...
export async function startConsumer() {
  ...
  try {
    ...
    const auditQueue = await channel.assertQueue('recommendation-updated-events', { durable: true });

    for (const eventName of consumedEvents) {
      await channel.bindQueue(queue.queue, process.env.RABBITMQ_EXCHANGE, eventName);
    }
    await channel.bindQueue(auditQueue.queue, process.env.RABBITMQ_EXCHANGE, 'RecommendationUpdated');
    ...
    await channel.consume(queue.queue, async (message) => {
      ...
      try {
        ...
        const recommendation = await refreshFromEvent(payload.event || eventName, payload);
        channel.ack(message);
        console.log(`Processed ${eventName} for student ${recommendation.studentId}`);
      } catch (error) {
